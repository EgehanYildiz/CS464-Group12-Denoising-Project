...
def prepare_data(data_dir='cifar10_data'):
    if not os.path.exists(data_dir):
        raise FileNotFoundError(f"Data directory {data_dir} not found. Please run cifar10_data_download.py first.")

    transform = transforms.ToTensor()
    
    print("Loading CIFAR-10 training and test datasets...")
    try:
        ...
        if "not found" in str(e):
            raise FileNotFoundError("CIFAR-10 dataset not found. Please run cifar10_data_download.py first.") from e
        raise e
